  ...
  List<Task> _filterTasksForToday(List<Task> tasks) {
    ...
    final filtered = tasks.where((task) {
      ...
      if (task.date != null && task.date!.isNotEmpty) {
        ...
        log('[date] ${task.title} | date="${task.date}" | today="$todayDateStr" | match=$match');
        return match;
      }
      ...
      log('[repeat] ${task.title} | repeatDays=${task.repeatDays} | weekday=$todayWeekday | match=$match');
      return match;
    }).toList();
        ...
        child: ListView(
          ...
          children: [
            ...
            if (todayTasks.isEmpty)
              ...
              ...todayTasks.asMap().entries.map((e) => ScheduleItem(
                task: e.value,
